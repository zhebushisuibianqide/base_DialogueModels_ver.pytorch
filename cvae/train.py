...
def train(model, data_iterator, optimizer, criterion, device, config):
    model.train()
    epoch_loss = 0
    kld_loss = 0.0
    bow_loss = 0.0
    for i, batch in enumerate(tqdm(data_iterator)):
        ...
        pred, kld, bow_logit = model(srcs, tgts, True, False) # use tercher forcing, recognize
        ...
        if config.use_kla:
            kl_weight = min([config.kl_step/config.kla_steps, 1.0])
            config.kl_step += 1
        else:
            kl_weight = 1.0

        loss_entory = criterion(pred, tgts)
        loss_kld = kl_weight * kld

        if config.use_bow:
            bow_logit = bow_logit.view(-1, pred_dim)
            loss_bow = criterion(bow_logit, tgts)
            bow_loss += loss_bow
            loss = loss_entory/len(batch) + loss_kld + loss_bow
        else:
            loss = loss_entory/len(batch) + loss_kld

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        epoch_loss += loss_entory.item()
        kld_loss += kld


    return epoch_loss, kld_loss, bow_loss
...
def train_on_batch(model, data_iterator, epoch, optimizer, criterion, device):
    ...
    for i, batch in enumerate(tqdm(data_iterator)):
        ...
        # pred = [batch_size, tgt_len, pred_dim]
        pred = model(srcs, tgts, True, False) #
...
def evaluate(model, data_iterator, criterion, device):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(data_iterator)):
            ...
            output, _, _ = model(srcs, tgts, True, False)  # turn on teacher forcing, use recognize
...
def inferring(model, data_iterator, criterion, id2word, device):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(data_iterator)):
            ...
            tgts = torch.LongTensor(tgts).to(device)
            # output = [batch_size, tgt_len, output_dim]
            output, _, _ = model(srcs, tgts, False, True)  # turn off teacher forcing, use priori possibility
...
def main():
    ...
    model = CVAE(Conf, device).to(device)
    ...
    if Conf.eval_per_batch is not None:
        ...
        for epoch in range(Conf.total_epoch_num):
            ...
            train_data_iterator, train_tgt_lens = prepare_batch_iterator(train_data, \
                                                                         train_length_data['tgt'], Conf.batch_size,
                                                                         shuffle=True)
            train_loss, kld_loss, bow_loss = train(model, train_data_iterator, optimizer, criterion, device, \
                                                   Conf)
            average_train_loss = train_loss / (sum(train_tgt_lens)-len(train_tgt_lens))
            average_kld_loss = kld_loss / len(train_data_iterator)
            average_bow_loss = bow_loss / (sum(train_tgt_lens)-len(train_tgt_lens))
            train_ppl = math.exp(average_train_loss)
            ...
            Train kld: {average_kld_loss:.3f} | Train Bow: {average_bow_loss:.3f}')
            print(f'\t Val. Loss: {average_valid_loss:.3f} |  Val. PPL: {valid_ppl:7.3f}')
            print(f'\t Val. infer Loss: {average_infer_loss:.3f} |  Val. infer PPL: {infer_ppl:7.3f}')
